...
object Semigroupals extends App {
  trait MySemigroupal[F[_]] {
    def product[A, B](fa: F[A], fb: F[B]): F[(A, B)] // fundamental method of Semigroupal
  }

  // the purpose of Semigroupal is to unwrap and wrap the items back, same as the case of Semigroup
  ...
  val aNoneTupled = optionSemigroupal.product(Some(123), None) // None - if any one item is None, the result will be None
  ...
  val aTupledList = Semigroupal[List].product(List(1, 2), List("a", "b")) // returns the List of the cartesian product of 1,2 and a,b (instead of zipping the result)
  println(aTupledList)
  ...
  import cats.syntax.functor._ // extension methods for flatMap applicable to monad
  ...
  // from Monads.scala
  // as we can safely define the product method from map and flatMap, we can say MyMonad can safely extend MySemigroupal
