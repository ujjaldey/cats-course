...
object Kleislis extends App {
  ...
  import cats.Id // Id[A] = A

  type InterestingKleisli[A, B] = Kleisli[Id, A, B] // wrapper over A => Id[B]
  // InterestingKleisli == Reader. if you see the source, you will see:
  // type Reader[-A, B] = ReaderT[Id, A, B]
  // type ReaderT[F[_], -A, B] = Kleisli[F, A, B]

  // hint
  val times = Kleisli[Id, Int, Int](x => x * 2)
  val plus4 = Kleisli[Id, Int, Int](y => y + 4)
  val composed = times.flatMap(t2 => plus4.map(p4 => t2 + p4))
  val composedFor = for {
    t2 <- times
    p4 <- plus4
  } yield t2 + p4
  ...
  val timesR = Reader[Int, Int](x => x * 2)
